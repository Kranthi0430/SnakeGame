...
public class Panel extends JPanel implements ActionListener {
    ...
    public void paintComponent(Graphics graphic) {
        super.paintComponent(graphic); draw(graphic);
    }
...
public void gameover(Graphics graphic) { //final score
     graphic.setColor(Color.white);
     graphic.setFont(new Font("Comic sans", Font.BOLD, 30));
     FontMetrics fme = getFontMetrics(graphic.getFont());
     graphic.drawString("Score:" + score, (width - fme.stringWidth("Score" + score)) / 2, graphic.getFont().getSize()); //gameover text
     graphic.setColor(Color.red);
     graphic.setFont(new Font("Comic sans", Font.BOLD, 60));
     FontMetrics fme1 = getFontMetrics(graphic.getFont());
     graphic.drawString("GAME OVER", (width - fme1.stringWidth("GAME OVER")) / 2, height/2); // to display the replay button
     graphic.setColor(Color.green);
     graphic.setFont(new Font("Comic sans", Font.BOLD, 30));
...
public void hit() {
    for (int i = length; i > 0; i--) { // to check hit with its own body
...
public void move() { // updating the coordinates of the body except the head
...
public class key extends KeyAdapter{ @Override public void keyPressed(KeyEvent e) {
    switch(e.getKeyCode()) {
        case KeyEvent.VK_UP: if (dir!='D') {
            dir = 'U';
        }
        break;
        case KeyEvent.VK_DOWN: if (dir!='U') {
            dir = 'D';
        }
        break;
        case KeyEvent.VK_LEFT: if (dir!='R') {
            dir = 'L';
        }
        break; case KeyEvent.VK_RIGHT: if (dir!='L') {
            dir = 'R';
        }
        break;
        case KeyEvent.VK_R: if (flag==false) {
            score = 0;
            length = 3;
            dir = 'R';
